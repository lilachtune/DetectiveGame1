## Character09.gd
## Персонаж: Сергей Громов
extends "res://scripts/CharacterBase.gd"

func _setup_character() -> void:
	character_id     = "character_09"
	character_sprite = preload("res://assets/characters/character_09.png")

	## Диалоги: заполни под свой сюжет.
	## Используй DialogueData.make_dialogue() для удобства.
	dialogues = [
		{
			"id":           "dlg_09_intro",
			"character_id": "character_09",
			"lines": [
				{
					"speaker":       "Сергей Громов",
					"text":          "Здравствуйте, инспектор. Чем могу помочь?",
					"portrait_path": "res://assets/characters/character_09_portrait.png"
				},
				{
					"speaker":       "Инспектор",
					"text":          "Мне нужно задать вам несколько вопросов.",
					"portrait_path": ""
				},
			]
		},
	]
