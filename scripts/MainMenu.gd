...
## ────────────────────────────────────────────────────────────────────────
## ФОН ГЛАВНОГО МЕНЮ (картинка)
## ────────────────────────────────────────────────────────────────────────
## Чтобы поставить свою картинку фоном главного меню — два варианта:
##
## Вариант А (без правок кода, проще всего):
##   1. Подготовь PNG размером 1920×1080.
##   2. Назови файл "menu_background.png".
##   3. Положи его в папку assets/ui/  (создай папку, если её нет):
##        res://assets/ui/menu_background.png
##   4. Готово — при запуске игры картинка подхватится автоматически,
##      без открытия редактора.
##
## Вариант Б (через редактор Godot):
##   1. Открой scenes/MainMenu.tscn.
##   2. Выдели корневой узел "MainMenu".
##   3. В Инспекторе найди поле "Background Image" (ниже) и
##      назначь туда свой PNG-файл.
##   Этот вариант имеет приоритет над Вариантом А.
##
## Если картинка не назначена и файл по умолчанию не найден — меню
## показывает фон-градиент (узел Background) как и раньше.
